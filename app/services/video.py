...
def _get_effective_video_codec(preferred_codec: str | None = None) -> str:
    ...
    如果用户未显式指定，优先自动检测系统 GPU 硬件编码器 (h264_nvenc, h264_qsv, h264_amf)。
    如果硬件编码不可用或之前在运行时失败过，则自动平滑回退到 libx264 (CPU)。
    """
    selected_codec = (preferred_codec or _get_configured_video_codec()).strip()

    ffmpeg_binary = utils.get_ffmpeg_binary()

    # 如果用户配置为默认/未设置/libx264，优先尝试自动检测 NVIDIA NVENC / Intel QSV / AMD AMF
    if not selected_codec or selected_codec == _DEFAULT_VIDEO_CODEC or selected_codec == "__default__":
        for gpu_codec in ("h264_nvenc", "h264_qsv", "h264_amf", "h264_mf", "h264_videotoolbox"):
            if gpu_codec not in _runtime_disabled_video_codecs and _ffmpeg_encoder_exists(ffmpeg_binary, gpu_codec):
                logger.info(f"Auto-detected hardware video encoder: {gpu_codec}")
                return gpu_codec
        return _DEFAULT_VIDEO_CODEC

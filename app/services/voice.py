...
def get_voice_recommendation(video_subject: str, script: str = "", language: str = "es") -> dict:
    """
    Recomienda la voz y parámetros de audio más adecuados según el tema o género (ej. Terror, Historia, Noticias).
    """
    text_to_analyze = f"{video_subject} {script}".lower()

    horror_keywords = [
        "terror", "miedo", "fantasma", "misterio", "leyenda", "creepypasta",
        "muerte", "sangre", "horror", "oscuridad", "paranormal", "monstruo",
        "asesino", "noche", "demonio", "pesadilla", "aparicion", "espectro", "tumba"
    ]
    history_keywords = [
        "historia", "ciencia", "documental", "universo", "espacio", "biografia",
        "descubrimiento", "planeta", "tecnologia", "fisica", "pasado", "antiguo", "imperio"
    ]
    business_keywords = [
        "noticias", "dinero", "finanzas", "economia", "negocios", "mercado",
        "inversion", "crypto", "bitcoin", "dolar", "empresa", "millonario"
    ]
    motivation_keywords = [
        "motivacion", "exito", "fitness", "gym", "superacion", "habitos",
        "mente", "disciplina", "estilo de vida", "meta", "superar"
    ]

    is_english = any(lang_code in str(language).lower() for lang_code in ["en", "en-us", "en-gb"])

    if any(kw in text_to_analyze for kw in horror_keywords):
        if is_english:
        ...
        return {
            ...
            "voice_name": voice_name,
            "alternative_voice": "es-ES-AlvaroNeural",
            "recommended_rate": 0.88,
            "recommended_bgm_volume": 0.15,
            "reason": "Voz grave, sombría y pausada ideal para transmitir suspenso y clima nocturno de terror.",
        }

    elif any(kw in text_to_analyze for kw in history_keywords):
        voice_name = "en-GB-RyanNeural" if is_english else "es-ES-AlvaroNeural"
        return {
            "category": "Documental / Historia",
            "voice_name": voice_name,
            "alternative_voice": "es-US-AlonsoNeural",
            "recommended_rate": 0.95,
            "recommended_bgm_volume": 0.20,
            "reason": "Tono narrativo maduro, solemne y formal de estilo documental.",
        }

    elif any(kw in text_to_analyze for kw in business_keywords):
        voice_name = "en-US-GuyNeural" if is_english else "es-MX-DaliaNeural"
        return {
            "category": "Noticias / Negocios",
            "voice_name": voice_name,
            "alternative_voice": "es-ES-ElviraNeural",
            "recommended_rate": 1.0,
            "recommended_bgm_volume": 0.20,
            "reason": "Voz clara, articulada y directa para temas profesionales y de noticias.",
        }

    elif any(kw in text_to_analyze for kw in motivation_keywords):
        voice_name = "en-US-EricNeural" if is_english else "es-MX-JorgeNeural"
        return {
            "category": "Motivación / Desarrollo Personal",
            "voice_name": voice_name,
            "alternative_voice": "es-CO-GonzaloNeural",
            "recommended_rate": 1.0,
            "recommended_bgm_volume": 0.25,
            "reason": "Tono firme, enérgico y seguro que transmite determinación.",
        }

    voice_name = "en-US-JennyNeural" if is_english else "es-MX-JorgeNeural"
    return {
        "category": "General / Narración",
        "voice_name": voice_name,
        "alternative_voice": "es-MX-DaliaNeural",
        "recommended_rate": 1.0,
        "recommended_bgm_volume": 0.20,
        "reason": "Voz versátil con excelente pronunciación neutra.",
    }

...
def _generate_response(prompt: str, app_config=None) -> str:
    try:
        ...
        try:
            ...
            if llm_provider == "moonshot" and ("401" in exc_str or "invalid_authentication" in exc_str):
                alt_url = "https://api.moonshot.ai/v1" if "moonshot.cn" in base_url else "https://api.moonshot.cn/v1"
                logger.warning(
                    f"moonshot 401 authentication error on {base_url}. "
                    f"Attempting fallback to international endpoint {alt_url}"
                )
                client = OpenAI(api_key=api_key, base_url=alt_url)
                response = client.chat.completions.create(
                    model=model_name, messages=[{"role": "user", "content": prompt}]
                )
                # Save working base_url for future calls
                config.app["moonshot_base_url"] = alt_url
            else:
                raise
        if response:
            if isinstance(response, ChatCompletion):

...
class TestVoiceRecommendation(unittest.TestCase):

    def test_voice_recommendation_horror(self):
        rec = vs.get_voice_recommendation("Una historia de terror y fantasmas en la noche")
        self.assertEqual(rec["category"], "Terror / Suspenso")
        self.assertEqual(rec["voice_name"], "es-MX-JorgeNeural")
        self.assertEqual(rec["recommended_rate"], 0.88)

    def test_voice_recommendation_history(self):
        rec = vs.get_voice_recommendation("La historia y ciencia del universo antiguo")
        self.assertEqual(rec["category"], "Documental / Historia")
        self.assertEqual(rec["voice_name"], "es-ES-AlvaroNeural")

    def test_voice_recommendation_default(self):
        rec = vs.get_voice_recommendation("Un tema curioso sobre mascotas")
        self.assertEqual(rec["category"], "General / Narración")
        self.assertIn("Jorge", rec["voice_name"])

...
st.set_page_config(
    page_title="MoneyPrinterModif",
    page_icon="🤖",
    layout="wide",
    initial_sidebar_state="auto",
    menu_items={
        "Report a bug": "https://github.com/speed205-ctrl/MoneyPrinter-modif/issues",
        "About": "# MoneyPrinterModif\nSimply provide a topic or keyword for a video, and it will "
        "automatically generate the video copy, video materials, video subtitles, "
        "and video background music before synthesizing a high-definition short "
        "video.\n\nhttps://github.com/speed205-ctrl/MoneyPrinter-modif",
    },
)
...
def _initialize_session_state():
    ...
    defaults = {
        ...
        "video_script": "",
        "video_terms": "",
        "video_script_prompt": config.app.get("video_script_prompt", ""),
        "custom_system_prompt": (
            config.app.get("custom_system_prompt")
            or llm.DEFAULT_SCRIPT_SYSTEM_PROMPT
        ),
...
def _render_brand(available_update: str | None = None):
    ...
    st.markdown(
        f"""
        <h1 class="mpt-brand">
            <span class="mpt-brand__name">MoneyPrinterModif</span>
            <a class="mpt-brand__version"
               href="https://github.com/speed205-ctrl/MoneyPrinter-modif"
               target="_blank"
               rel="noopener noreferrer"
               aria-label="Open MoneyPrinterModif on GitHub"
...
def reset_script_system_prompt():
    ...
    _set_runtime_config("app", "custom_system_prompt", "")
...
def _render_script_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            with st.expander("💡 Generador de Ideas y Títulos Virales con IA", expanded=False):
                niche_input = st.text_input(
                    "Nicho o Temática (ej. Terror, Suspenso, Leyendas Urbanas, Historia)",
                    value="Terror y Suspenso",
                    key="idea_niche_input",
                )
                if st.button("✨ Generar Ideas Virales", key="generate_ideas_button", use_container_width=True):
                    with st.spinner("Generando títulos e ideas virales con el Agente IA..."):
                        idea_agent = IdeaGeneratorAgent(default_language=params.video_language or "es")
                        ideas = idea_agent.generate_ideas(
                            niche=niche_input,
                            count=5,
                            language=params.video_language,
                        )
                        st.session_state["generated_ideas_list"] = ideas

                if "generated_ideas_list" in st.session_state and st.session_state["generated_ideas_list"]:
                    st.markdown("**Ideas Sugeridas por el Agente:**")
                    for idx, idea in enumerate(st.session_state["generated_ideas_list"]):
                        title = idea.get("title", "")
                        hook = idea.get("hook", "")
                        st.markdown(f"**{idx + 1}. {title}**")
                        st.caption(f"_{hook}_")
                        def _apply_selected_idea(t=title, h=hook):
                            st.session_state["video_subject"] = f"{t}: {h}"

                        st.button(
                            f"👉 Usar esta idea para el video",
                            key=f"use_idea_{idx}",
                            on_click=_apply_selected_idea,
                        )
            ...
            with st.container(key="advanced_settings_script"):
                with st.expander(tr("Advanced Script Settings"), expanded=False):
                    ...
                    _set_runtime_config(
                        "app", "video_script_prompt", params.video_script_prompt
                    )
                    ...
                    _set_runtime_config(
                        "app", "custom_system_prompt", system_prompt
                    )
